            ...
            <div v-else-if="logList.length" class="log-list-wrap">
              <v-list density="comfortable" nav class="log-list">
                ...
                >
                  ...
                  <template #append>
                    <v-chip v-if="logList[0]?.id === item.id" size="x-small" variant="tonal" color="primary">
                      最新
                    </v-chip>
                  </template>
                </v-list-item>
          ...
          <div class="log-content-shell">
            <div class="shell-header shell-header--content">
              <div>
                ...
                <div class="shell-caption">
                  {{ selectedLog?.runAt ? `当前日志：${selectedLog.runAt}` : '请选择左侧日志文件查看详细内容' }}
                </div>
              </div>
              <div>
              <v-btn
            ...
            <div class="log-content-wrap">
              <div v-if="!selectedLogId" class="panel-state panel-state--content-empty">
                ...
                <div class="panel-state__desc">请从左侧选择一条日志记录，系统将在右侧显示完整内容。</div>
              </div>
...
const selectedLog = computed(() => logList.value.find((item) => item.id === selectedLogId.value) || null)
const contentLineCount = computed(() => (fContent.value ? fContent.value.split('\n').length : 0))
const contentCharCount = computed(() => fContent.value.length)
