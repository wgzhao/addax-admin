    ...
    <v-card-text class="table-panel__content">
      <div class="summary-grid">
        <div class="summary-card">
          ...
          <strong>{{ latestResult?.bizDate || '-' }}</strong>
        </div>
        <div class="summary-card">
        ...
        <div class="summary-card">
          ...
          <strong :class="{ 'text-error': totalErrors > 0 }">{{ formatNumber(totalErrors) }}</strong>
        </div>
        <div class="summary-card">
      ...
      <div v-else-if="results.length" class="result-table-shell ds-table-wrap">
        ...
        <v-data-table-virtual
          ...
          class="result-table"
        >
          <template #item.bizDate="{ item }">
            <span class="cell-emphasis">{{ item.bizDate || '-' }}</span>
          </template>
...
const totalRecords = computed(() => results.value.reduce((sum, item) => sum + Number(item.totalRecs || 0), 0))
const totalErrors = computed(() => results.value.reduce((sum, item) => sum + Number(item.totalErrors || 0), 0))
const peakRecSpeed = computed(() => results.value.reduce((max, item) => Math.max(max, Number(item.recSpeed || 0)), 0))
...
const exportCsv = () => {
  ...
  const rows = results.value.map((row) => columns.map((key) => escapeCsvCell((row as Record<string, unknown>)[key])).join(','))
