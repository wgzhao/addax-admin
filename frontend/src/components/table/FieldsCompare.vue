...
const normalizeType = (value?: string) => String(value || '').replace(/\s+/g, '').toLowerCase()
...
const mappedFields = computed(() =>
  fields.value.map((item) => {
    ...
    ].filter(Boolean).join(' / ') || '无长度与精度信息'

    const tableDescriptor = item.tblComment || '无表备注'
    const hasDiff = normalizeType(item.sourceType) !== normalizeType(item.targetTypeFull || item.targetType)
