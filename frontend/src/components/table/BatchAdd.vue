        ...
        <v-stepper-window-item :value="1">
          <div class="step-layout">
            <div class="step-hero">
              ...
              <div class="step-desc">先选源系统，再选对应数据库。完成后再进入表选择，避免第二步信息过载。</div>
            </div>
            ...
            <Transition name="fade">
              <v-alert
                ...
                class="step-alert"
              >
                当前已锁定 <strong>{{ selectedSourceId.code }} · {{ selectedSourceId.name }}</strong>
                与数据库 <strong>{{ selectedDb }}</strong>，点击「下一步」加载待采集表。
              </v-alert>
            </Transition>
          </div>
        ...
        <v-stepper-window-item :value="2">
          <div class="step-layout">
            <div class="step-hero">
              ...
              <div class="step-desc">把搜索、选择反馈和表清单放在同一层，用户可以快速定位重点而不是淹没在表格里。</div>
            </div>
            ...
            <div v-else-if="tables.length > 0" class="table-stage">
              <div class="table-stage__toolbar">
                ...
                <div class="table-stage__toolbar-right">
                  <v-chip color="primary" variant="tonal" size="small">已选 {{ selectedCnt }}</v-chip>
            ...
            <div v-else class="panel-state">
              ...
              <div class="panel-state__desc">可以返回上一步重新选择数据源，或确认当前数据库是否已经全部纳入采集。</div>
            </div>
          </div>
        </v-stepper-window-item>
        ...
        <v-stepper-window-item :value="3">
          <div class="step-layout step-layout--config">
            <div class="step-hero">
              ...
              <div class="step-desc">把真正影响落库结果的配置放在主区，把命名预览固定在右侧，避免提交前失焦。</div>
            </div>

            <v-alert type="info" variant="tonal" density="comfortable" icon="mdi-table-multiple" class="step-alert">
                ...
                <section class="stage-panel">
                  <div class="stage-panel__header">
                    <div>
                      ...
                      <div class="stage-panel__caption">优先处理目标端、目标库和目标表模板，这是本步骤最核心的三项。</div>
                    </div>
                  </div>
                  <v-row dense>
                    <v-col cols="12" md="4">
                ...
                <section class="stage-panel">
                  <div class="stage-panel__header">
                    <div>
                      ...
                      <div class="stage-panel__caption">这些配置决定了目标表的分区方式、格式与压缩策略，作为第二层信息展示。</div>
                    </div>
                  </div>
                  <v-row dense>
                    <v-col cols="12" md="6">
              ...
              <aside class="config-side">
                <section class="preview-card">
                  <div class="preview-card__header">
                    ...
                    <v-chip size="small" variant="tonal" color="primary">预览 {{ Math.min(selectedCnt, PREVIEW_LIMIT) }} 项</v-chip>
                  </div>

                  <div class="preview-highlights">
                    <v-chip size="small" variant="outlined">目标库 {{ targetDb || '-' }}</v-chip>
                    <v-chip size="small" variant="outlined">模板 {{ targetTableTemplate || '${table}' }}</v-chip>
                  </div>
                    ...
                    <table class="preview-table">
                      ...
                      <tbody>
                        ...
                        <tr v-if="selectedCnt > PREVIEW_LIMIT">
                          <td colspan="3" class="more-row">…还有 {{ selectedCnt - PREVIEW_LIMIT }} 张表未展开</td>
                        </tr>
        ...
        <v-stepper-window-item :value="4">
          <div class="success-shell">
            ...
            <div class="success-meta">
              <v-chip size="small" variant="tonal" color="success">目标库 {{ targetDb || '-' }}</v-chip>
...
const selectedSourceLabel = computed(() =>
  selectedSourceId.value ? `${selectedSourceId.value.code} · ${selectedSourceId.value.name}` : '未选择',
)
...
.overview-card {
  ...
  background:
    linear-gradient(180deg, rgba(var(--v-theme-primary), 0.06), rgba(var(--v-theme-primary), 0.01)),
