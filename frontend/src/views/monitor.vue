  ...
  <v-container fluid class="pa-6">
    <v-row dense>
      ...
      <v-col cols="12" md="12">
        <v-card flat :title="accomplishListTable.title" class="mb-4">
          <v-card-text>
            <v-data-table :items="data.accomplishList" :headers="accomplishListTable.headers" density="compact"
              :sort-by="accomplishListTable.sortBy" class="elevation-1" />
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
    ...
    <v-row dense>
      ...
      <v-col cols="12" md="7">
        <v-card flat :title="specialTaskTable.title" class="mb-4">
          <v-card-text>
            <v-data-table :items="data.specialTask" :headers="specialTaskTable.headers" density="compact"
              :sort-by="specialTaskTable.sortBy" class="elevation-1" />
          </v-card-text>
        </v-card>
      </v-col>
        ...
        <v-row>
          <v-col cols="12" md="12">
            <v-card flat :title="rejectTaskTable.title" class="mb-4">
              <v-card-text>
                <v-data-table :items="data.rejectTask" :headers="rejectTaskTable.headers" density="compact"
                  :sort-by="rejectTaskTable.sortBy" class="elevation-1" hide-no-data hide-default-footer />
              </v-card-text>
            </v-card>
      </v-col>
      <!-- 短信发送情况 -->
      <v-col cols="12" md="12">
        <v-card flat :title="smsDetailTable.title" class="mb-4">
          <v-card-text>
            <v-data-table :items="data.smsDetail" :headers="smsDetailTable.headers" density="compact"
              :sort-by="smsDetailTable.sortBy" class="elevation-1" hide-no-data hide-default-footer />
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
    ...
    <v-col cols="12" md="12">
      <v-card flat :title="fieldChangeTable.title" class="mb-4">
        <v-card-text>
          <v-data-table-server :items="fieldChangeItems" :headers="fieldChangeTable.headers" density="compact"
            :items-per-page="fieldChangePageSize" :items-length="fieldChangeTotal" :loading="fieldChangeLoading"
            @update:options="loadFieldChangeItems" class="elevation-1" />
        </v-card-text>
      </v-card>
    </v-col>
    </v-row>
    ...
    <v-col cols="12" md="12">
      <v-card flat :title="sysRiskTable.title" class="mb-4">
        <v-card-text>
          <v-data-table :items="data.sysRisk" :headers="sysRiskTable.headers" density="compact"
            :sort-by="sysRiskTable.sortBy" class="elevation-1"  hide-no-data hide-default-footer />
        </v-card-text>
      </v-card>
    </v-col>
  </v-container>
...
const getData = async () => {
  try {
    const [accomplishList, specialTask, rejectTask, sysRisk, smsDetail] =
      await Promise.all([
...
onMounted(() => {
  getData()
  // 触发表格首次加载
  loadFieldChangeItems({ page: 1, itemsPerPage: fieldChangePageSize.value, sortBy: fieldChangeCurrentSort.value })
})
</script>
<route lang="json">{
  "meta": {
