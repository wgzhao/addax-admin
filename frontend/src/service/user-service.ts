...
class UserService {
  ...
  update(username: string, payload: UpdateUserPayload): Promise<AccountUser> {
    return Requests.put(
      `${this.prefix}/${encodeURIComponent(username)}`,
      payload
    ) as unknown as Promise<AccountUser>;
  }
  ...
  enable(username: string): Promise<AccountUser> {
    return Requests.post(
      `${this.prefix}/${encodeURIComponent(username)}/enable`
    ) as unknown as Promise<AccountUser>;
  }

  disable(username: string): Promise<AccountUser> {
    return Requests.post(
      `${this.prefix}/${encodeURIComponent(username)}/disable`
    ) as unknown as Promise<AccountUser>;
  }
}
