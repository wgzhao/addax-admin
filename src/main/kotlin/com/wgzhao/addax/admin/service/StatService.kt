...
class StatService(
    ...
    private val log = LoggerFactory.getLogger(StatService::class.java)

    // 按采集源统计最近一次采集的数据量
    fun statDataBySource(): List<Map<String, Any>> {
        val sql = """
                select
                b.code ,
    ...
    fun statTotalData(): Double {
        val sql = """
                ...
                """.trimIndent()
        return jdbcTemplate.queryForObject(sql, Double::class.java) ?: 0.0
    }

    // 最近 12个月的采集累计数据量，单位为 GiB
    fun statLast12MonthsData(): List<Map<String, Any>> {
        val sql = """
                select
    ...
    // 按采集源统计最近一次采集的耗时
    fun statTimeBySource(): List<Map<String, Any>> {
        val sql = """
                select
                code ,
    ...
    fun statLast5DaysTimeBySource(): List<Map<String, Any>> =
        etlStatisticRepo.findLast5DaysTakeTimes() ?: emptyList()

    //按采集源统计目前的采集状态统计
    fun statStatusBySource(): List<Map<String, Any>> {
        val sql = """
                 SELECT
                    code,
                    jsonb_build_object(
    ...
    // 最近采集的完成率
    fun statLastAccompRatio(): List<Map<String, Any>> {
        val sql = """
                SELECT
    ...
    fun getLast15Records(tid: Long): List<EtlStatistic?>? {
