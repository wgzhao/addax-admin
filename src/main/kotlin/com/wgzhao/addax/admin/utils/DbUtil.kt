...
object DbUtil {
    ...
    fun getColumnComment(conn: Connection, dbName: String?, tableName: String?, columnName: String?): String {
        try {
            val dbType = conn.getMetaData().getDatabaseProductName().lowercase(Locale.getDefault())
            var sql: String? = null
     ...
     * @return 表注释
     */
    fun getTableComment(conn: Connection, dbName: String?, tableName: String?): String {
        try {
            val dbType = conn.getMetaData().getDatabaseProductName().lowercase(Locale.getDefault())
            var sql: String? = null
            if (dbType.contains("mysql")) {
    ...
    // 获取表注释的另外一种实现
    fun getTableCommentAlt(connection: Connection, url: String, dbName: String?, tableName: String?): String? {
        ...
        try {
            connection.prepareStatement(sql).use { ps ->
                ps.executeQuery().use { rs ->
                    if (rs.next()) {
                        result = Optional.ofNullable<String?>(rs.getString(1)).orElse("")
                    }
                }
            }
