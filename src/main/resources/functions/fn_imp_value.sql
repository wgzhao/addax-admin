CREATE OR REPLACE function STG01.fn_imp_value(i_kind in varchar2,i_sp_id in varchar2:='',i_value1 in varchar2:='')
   return clob
as
   o_return clob;
   ctmp1 clob;
...
begin
       ...
       if strtmp2 is not null then
          select replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(
                 o_return,'${sou_dbcon}',t.sou_db_constr)
                          ...
                          ,'${hdp_cols}',(select regexp_replace(xmlagg(xmlparse(content col_name||',' wellformed) order by col_idx).getclobval(),',$','') from stg01.tb_imp_tbl_hdp where tid=t.tid and col_name<>'LOGDATE' group by tid) )
                 into o_return
   ...
   --ODS���������������XDS������������
     ...
     select entry_content into ctmp1 from stg01.tb_dictionary where entry_code='5005' and entry_value='ETL_END_CHK' ;
     for tbl in(
       ...
       left join stg01.tb_dictionary c on c.entry_code='2014' and lower(c.entry_value)=lower(a.column_name)
       where t.bvalid = 1 and t.sysid = i_sp_id and t.sysid <> 'JY'
       group by t.tid,t.dest )
     loop
       o_return := o_return || replace(replace(replace(replace(ctmp1,'${dest_part}',stg01.fn_imp_value('dest_part_value',tbl.tid)),'${tbl}',tbl.dest),'${otbl}',regexp_replace(tbl.dest,'^ods','xds')),'${cols}',tbl.cols) || chr(10) ;
     end loop ;
   ...
   --������logdate������������
      ...
      select nvl(a.param_value, '1')
             into o_return
       from stg01.vw_imp_etl t
       left join stg01.vw_imp_param a
   ...
   --������������������
      ...
      select spname
             into o_return
       from (select spname from stg01.vw_imp_etl where tid = i_sp_id
             union all
             select spname from stg01.vw_imp_sp where sp_id = i_sp_id
             union all
             select spname from stg01.vw_imp_plan where pn_id = i_sp_id
             union all
             select ds_name from vw_imp_ds2_mid where i_sp_id in(ds_id,tbl_id) and rownum=1
             union all
             select sysid||'_'||sys_name from stg01.vw_imp_system where sysid = i_sp_id
            );
   ...
   --ds_sql:���������������������������������������������������������
      ...
      for c1 in (select 'create or replace view ds.' || t.dsview || ' as ' || chr(10) ||
                        stg01.fn_imp_param_replace(regexp_replace(t.sou_table, ';$', '') || ';',
                                                   t.param_sou) ds_sql
   ...
   --ds_json:���������������datax������JSON���
       ...
       with t_map as --���������������������������
          (select distinct col_map,regexp_substr(regexp_substr(col_map, '(as|AS) [^;]+', 1, level), '\w+$') col_map_name,
                      ...
                      from (select col_map from stg01.vw_imp_ds2_mid where col_map is not null and tbl_id = i_sp_id group by col_map)
          ...
          t_db as (select tbl_id,col_name,col_define,col_type,row_number()over(partition by tbl_id,col_name order by kind desc) px
                  from(select 0 kind, t.tbl_id, col_name, col_name col_define, col_type
                         from stg01.vw_imp_ds2_mid t
                        inner join stg01.tb_imp_etl_tbls a on decode(t.sou_istab , 1 , lower(t.sou_table) , 'ds.'||t.dsview) = lower(a.db_name||'.'||a.tbl_name)
                        where t.tbl_id = i_sp_id and t.sou_ishdp = '1'
                        union all
                       select 0 kind,x.tbl_id, a.column_name, a.column_name, a.data_type
                         from stg01.vw_imp_etl t
                        inner join stg01.vw_imp_ds2_mid x on x.tbl_id = i_sp_id and decode(sou_istab , 1 , lower(sou_table) , 'ds.'||dsview) = lower(t.dest) and x.sou_ishdp = '0'
                        inner join stg01.tb_imp_etl_soutab a on a.sou_db_conn=t.sou_db_conn and upper(a.owner)=upper(t.sou_owner) and upper(a.table_name)=upper(t.sou_tablename)
                        union all
          ...
          t_sql as (select t.tbl_id,
                           ...
                           case when t.sou_ishdp = 0 then ' from ' ||
                                ...
                                when t.sou_istab = 1 then ' from '||t.sou_table||' where '||replace( nvl(t.sou_filter,'logdate=''${dest_part}''') , '${dest_part}' , nvl(to_char(stg01.fn_imp_value('dest_part',c.tid)),'${TD}') )
                    ...
                    --ODS���������������������������������TID������������������������������������
                    left join stg01.vw_imp_etl c on lower(c.dest) = lower(t.sou_table)
                    --������������������
                    left join stg01.tb_dictionary d on d.entry_code='2014' and upper(d.entry_value)=upper(b.column_name)
        ...
        select replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(to_clob(c.jobfile),'${s_conn}',a.s_conn),'${s_user}',a.s_user),'${s_pass}',a.s_pass),
                   '${s_sql}',stg01.fn_imp_param_replace('select '||a.s_cols||a.s_filter,t.param_sou)),
                   '${d_presql}',decode(case when t.retry_cnt<3 and t.pre_sql='T' then 'D' else t.pre_sql end,'T','truncate table @table','D','delete from @table','N','',stg01.fn_imp_param_replace(t.pre_sql,t.param_sou))),
                   '${d_postsql}',stg01.fn_imp_param_replace(t.post_sql,t.param_sou)), '${d_cols}',a.d_cols),
                   '${d_conn}',d_conn),'${d_user}',d_user),'${d_pass}',d_pass),'${d_tblname}',t.dest_owner||'.'||t.dest_tablename)
               into o_return
        from stg01.vw_imp_ds2_mid t
        left join t_sql a on a.tbl_id = t.tbl_id
        left join stg01.vw_imp_jobfile c on c.jobkind='H2'||t.db_kind
        where t.tbl_id=i_sp_id ;
   ...
   --ds_cmd:���������������������������������
       ...
       select case when db_kind_full = 'file' then
         ...
         left join (select tbl_id,
                           dest_owner dest_dir,
                           to_char(stg01.fn_imp_param_replace(dest_tablename,
                                                              param_sou)) dest_file,
                           to_char(stg01.fn_imp_param_replace(sou_table, param_sou)) sou_table,
   ...
   --�����������������������������
      ...
      for c1 in (select stg01.fn_imp_param_replace(replace(regexp_substr(dest_tablename,'/[^/.]+'),'/')||chr(9)||'${TD}'||chr(9)||'1'||chr(9)||'${NOW}',i_value1) flag from tb_imp_ds2_tbls where ds_id = i_sp_id)
      loop
   ...
   --������hadoop��������������������������������������������������������������
      ...
      select replace(wm_concat(create_db), ',', chr(10)) || chr(10)
       ...
       where t.sys_kind = 'etl' and t.bvalid = 1
         and t.sysid not in (select fid from stg01.tb_imp_flag where tradedate = getltd() and fval = 4 and kind = 'ETL_END');
      ...
      --���������:������������������
      for tbl in (select tid,lower(dest) tblname from stg01.vw_imp_etl where bcreate = 'Y' and bupdate = 'n' and bvalid = 1 and tid in(select tid from stg01.tb_imp_tbl_sou group by tid))
      loop
          ...
          --�����������
          select max(tbl_comment) into strtmp1 from stg01.tb_imp_tbl_sou where tid = tbl.tid and tbl_comment is not null ;
   ...
   --hadoop������������������(���������������������)
      ...
      for c1 in (select alter_sql from stg01.vw_imp_tbl_diff_mysql)
      loop
           o_return := o_return || c1.alter_sql || chr(10) ;
      end loop ;
   ...
   --------------------------------------------
      ...
      select to_char(replace(wm_concat(entry_value),',','|'))
             into strtmp1
      from stg01.tb_dictionary where entry_code='2020' ;
      ...
      for c1 in(select 'select cast(now() as varchar)||'':���������������������'||a.db_name||'.'||a.tbl_name||''';'||
                       ...
                       a.db_name||'.'||a.tbl_name||' where logdate=''' || stg01.fn_imp_value('dest_part_value', t.tid) || ''';' hsql
                from vw_imp_etl t
   ...
   --���������������������������������������������
      ...
      for c1 in(select 'drop table if exists '||a.db_name||'.'||a.tbl_name||
                       ...
                       'alter table ' || a.db_name||'.'||a.tbl_name ||' drop if exists partition(logdate=' ||stg01.fn_imp_value('dest_part_value', t.tid) || ');' ||
                       'alter table ' || a.db_name||'.'||a.tbl_name ||' add if not exists partition(logdate=' ||stg01.fn_imp_value('dest_part_value', t.tid) || ');' ||
   ...
   --���������TDH���������������
      ...
      for c1 in (select 'drop table if exists ods.'||a.db_name||'_'||a.tbl_name||';'||chr(10)||
                        ...
                        'alter table ods.' || a.db_name||'_'||a.tbl_name ||' add if not exists partition(etl_date=' ||stg01.fn_imp_value('dest_part_value', t.tid) || ');'||chr(10)||
                        'use ods;'||chr(10)||
   ...
   --���������TDH������������
       ...
       for c1 in(select '/mnt/dfs/xds/xds'||lower(sysid||'/'||dest_tablename)||'/logdate='||stg01.fn_imp_value('dest_part_value',tid)||'/*,'||
                        '/inceptor1/user/hive/warehouse/ods.db/add/'||lower(replace(dest,'.','_'))||'/etl_date='||stg01.fn_imp_value('dest_part_value',tid) hsql
                   from stg01.vw_imp_etl t
                  where btdh='Y' and t.bvalid=1 and t.sysid = i_sp_id)
       loop
                 o_return := o_return || c1.hsql || chr(10) ;
