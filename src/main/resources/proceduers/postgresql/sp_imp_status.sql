-- PostgreSQL version of sp_imp_status
CREATE OR REPLACE FUNCTION sp_imp_status(i_kind varchar, i_sp_id varchar)
RETURNS void AS $$
DECLARE
...
BEGIN
   ...
   SELECT max(t.remark || CASE WHEN length(i_kind) = 2 THEN E'\n' || b.remark ELSE '' END),
     ...
     FROM t_sp t
    INNER JOIN t_com b ON b.sp_id = t.sp_id
                      AND i_sp_id IN (b.sp_id, b.com_id)
                      AND COALESCE(b.flag, 'N') <> 'X'
   ...
   IF v_err >= 0 THEN
     ...
     IF length(v_kind) = 1 THEN
       ...
       UPDATE tb_imp_sp
          SET flag       = v_kind,
              ...
              end_time   = CASE WHEN v_kind = 'Y' THEN v_curtime
                               WHEN v_kind = 'E' THEN v_curtime
                               ELSE end_time END,
       ...
       IF v_kind = 'R' THEN
         ...
         UPDATE tb_imp_ds2_tbls
          ...
          WHERE ds_id = i_sp_id AND v_sou = 'ds'
            AND (
                (COALESCE(flag, 'N') <> 'X' AND (SELECT retry_cnt FROM tb_imp_ds2 WHERE ds_id = i_sp_id) = 3)
                OR
                (COALESCE(flag, 'E') IN ('E','R') AND (SELECT retry_cnt FROM tb_imp_ds2 WHERE ds_id = i_sp_id) < 3)
                );
       ...
       ELSIF v_kind = 'E' THEN
         ...
         FOR c1 IN (
           SELECT v_sou || ':' || spname || '执行失败!!' || E'\n' || '[' ||
                  ...
                  CASE WHEN v_sou IN ('ds','plan') THEN msg2 END AS msg,
                  mobile
             FROM (
               -- sp执行结束
               SELECT sp_id, t.spname, start_time, end_time, a.proj_name || ',1' AS mobile
                 FROM vw_imp_sp t
             ...
             LEFT JOIN (
               SELECT ds_id,
                      E'\n' || '失败任务' || sum(CASE WHEN flag NOT IN ('Y','X') THEN 1 ELSE 0 END) ||
                      '个(总任务' || count(1) || '个)' AS msg2
                 FROM (
                   ...
                   SELECT sp_id, COALESCE(flag, 'E'),
                          replace(substring(regexp_replace(com_text, '^[^' || E'\n' || ':,]+', ''), 1, 100), '#', '')
                     FROM tb_imp_sp_com
                    WHERE sp_id = i_sp_id AND v_sou = 'plan'
                 ) s GROUP BY ds_id
             ) a ON a.ds_id = t.sp_id
         ) LOOP
             PERFORM sp_sms(c1.msg, c1.mobile, '110');
         END LOOP;
     ...
     ELSE
       ...
       UPDATE tb_imp_sp_com
          SET flag       = substring(v_kind FROM 2 FOR 1),
              ...
              end_time   = CASE WHEN v_kind = 'cY' THEN v_curtime
                               WHEN v_kind = 'cE' THEN v_curtime
                               ELSE end_time END
     ...
     VALUES (v_sou, gettd(), v_kind, i_sp_id,
            ...
            ',执行耗时：' || to_char(trunc(extract(epoch from (CURRENT_TIMESTAMP - v_curtime))), 'FM99999999') ||
...
EXCEPTION
   WHEN OTHERS THEN
      PERFORM sp_sms('sp_imp_status执行报错,i_kind=[' || i_kind || '],i_sp_id=[' || i_sp_id ||
                     '],v_sou=[' || v_sou || '],v_kind=[' || v_kind || '],错误说明=[' || SQLERRM || ']',
                     '18692206867', '110');
      RAISE;
END;
$$ LANGUAGE plpgsql;
