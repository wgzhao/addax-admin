CREATE OR REPLACE procedure sp_imp_param(i_curr_date in integer default 1)
as
...
begin
       ...
       select max(init_date) into v_trade_date from vw_trade_date where init_date<= v_curr_date ;
       ...
       if v_param_sou = 'C' then
         v_strtmp := 'create or replace view vw_imp_param_all as select * from (select param_kind_0,param_value from tb_imp_param0 where param_sou=''C'') pivot (max(param_value) for param_kind_0 in(';
