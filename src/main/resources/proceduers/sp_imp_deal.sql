CREATE OR REPLACE procedure STG01.sp_imp_deal(i_kind in varchar2,i_key in varchar2 := '')
as
  ...
  if i_kind = 'git_deploy' and length(nvl(i_key,' '))=32 then
    insert into stg01.tmp_ci_deploy_add
      ...
      select nvl(a.sp_id, sys_guid()) sp_id,
        ...
        from t_change x
        left join stg01.tb_ci_deploy t
          on t.dep_id = i_key
         and x.file_name = lower(t.file_name)
        left join stg01.tb_imp_sp a
          on a.sp_owner || '.' || a.sp_name = regexp_replace(x.file_name, '\.sql$')
       where x.add_kind < 9
         and regexp_like(x.file_name, '^[a-z0-9_]+\.[a-z0-9_]+\.sql$') ;

    --新增脚本插入配置主表
    insert into stg01.tb_imp_sp(sp_id,sp_owner,sp_name,task_group)
    select sp_id,sp_owner,sp_name,
           case when sp_owner in('sjzl','qmfx') then sp_owner end task_group
      from stg01.tmp_ci_deploy_add
     where add_kind = 1 ;

    --复活脚本
    --update stg01.tb_imp_sp set flag='Y',start_time=sysdate
    --where sp_id in(select sp_id from stg01.tmp_ci_deploy_add where add_kind = 2) ;

    --置死脚本
    update stg01.tb_imp_sp set flag='X'
    where sp_id in(select sp_id from stg01.tmp_ci_deploy_add where add_kind = 3) ;

    --将脚本插入命令列表
    delete from stg01.tb_imp_sp_com where sp_id in(select sp_id from stg01.tmp_ci_deploy_add);
    for c1 in(select sp_id,com_text||chr(10)||'----shell----'||chr(10) com_text,regexp_count(nvl(com_text,' '),'\-{4}('||sqlengine||')\-{4}'||chr(10)) com_num from stg01.tmp_ci_deploy_add)
    loop
        --避免一个项目中不同目录出现重复名称
        delete from stg01.tb_imp_sp_com where sp_id = c1.sp_id ;
        ctmp1 := c1.com_text ;
    ...
    strtmp2 := stg01.fn_imp_value('get_schema');
    ...
    select t.sp_id,
      ...
      from stg01.tmp_ci_deploy_add t;

    --删除本次新增SP的涉及表及目标表
    delete from stg01.tb_imp_sp_needtab
    ...
    loop
              ...
              loop
                  ...
                  if strtmp1 is not null then
                      ...
                      select c1.sp_id,strtmp1,'ALL' from stg01.tb_imp_sp_needtab
    ...
    loop
              ...
              loop
                  ...
                  if strtmp1 is not null then
                      insert into stg01.tb_imp_sp_needtab(sp_id,table_name,kind)
                      select c1.sp_id,strtmp1,'DEST' from stg01.tb_imp_sp_needtab
    ...
    delete from stg01.tb_imp_sp_needtab where kind='NEEDS';
    insert into stg01.tb_imp_sp_needtab
      (sp_id, table_name, kind)
      select sp_id, needs_sp_id, 'NEEDS'
        from (select t.sp_id, a.sp_id needs_sp_id
                from stg01.tb_imp_sp_needtab t
               inner join stg01.vw_imp_sp a
    ...
    delete from stg01.tb_imp_sp_needtab where regexp_like(kind,'RUNLEV\d+') ;
    ----只依赖数据源的SP，即为初始层
    insert into stg01.tb_imp_sp_needtab(sp_id,kind)
    select sp_id,'RUNLEV0' from stg01.vw_imp_sp where bvalid=1 and sp_id not in(select sp_id from vw_imp_sp_needs where length(needs)=32);
    ...
    loop
      ...
      select t.sp_id,'RUNLEV'||c1
        from stg01.vw_imp_sp_needs t
        left join stg01.tb_imp_sp_needtab a on a.sp_id=t.needs and regexp_like(a.kind,'RUNLEV\d+')
       where length(t.needs)=32 and t.sp_id not in(select sp_id from stg01.tb_imp_sp_needtab where regexp_like(kind,'RUNLEV\d+'))
       group by t.sp_id
      having count(1)=sum(case when a.sp_id is not null then 1 else 0 end);
    end loop ;

    --SP穿透后的依赖
    delete from stg01.tb_imp_sp_needtab where kind='NDS';
    insert into stg01.tb_imp_sp_needtab(kind,sp_id,table_name)
    select 'NDS',sp_id,needs from stg01.vw_imp_sp_needs
    where length(needs)=32 and sp_id in(select sp_id from stg01.tb_imp_sp_needtab where kind like 'RUNLEV%');

    for c1 in 1..10
    loop
      insert into stg01.tb_imp_sp_needtab(kind,sp_id,table_name)
      select distinct kind,t.sp_id,a.needs from stg01.tb_imp_sp_needtab t
      inner join stg01.vw_imp_sp_needs a on a.sp_id=t.table_name and length(a.needs)=32
      where kind = 'NDS' and not exists(select 1 from stg01.tb_imp_sp_needtab where kind='NDS' and sp_id=t.sp_id and table_name=a.needs);
    end loop ;

    --更新SP依赖显示表，仅影响前台页面展示，具体的调起依赖，直接使用的源表
    delete from stg01.tb_imp_sp_needall ;
    insert into stg01.tb_imp_sp_needall
    with t_sp as
    ...
    select '1,'||wm_concat(proj_name)
           into strtmp2
    from (select proj_name from stg01.tb_ci_deploy where dep_id = i_key group by proj_name );

    stg01.sp_sms(strtmp1,strtmp2,'010');
...
exception
   when others then
        stg01.sp_sms('sp_imp_deal执行报错,kind=['||i_kind||'],key=['||i_key||'],错误说明=['||substr(sqlerrm,1,200)||']','18692206867','110') ;
end;