CREATE OR REPLACE procedure STG01.sp_sms(i_msg in varchar2,i_mobile in varchar2:='1',i_sendtype in varchar2:='010')
       --���������������
       --i_msg:���������������������
       --i_mobile:���������������������������������
       --i_sendtype:���������������������������������������KK���������������1������������0���������
       ...
       select to_char(wm_concat(mobile))
         ...
         from (select distinct nvl(a.mobile, t.gp) mobile
                 ...
                 left join stg01.vw_mobile_group a
                   on a.groupid = t.gp)
        where mobile in(select mobile from stg01.vw_mobile_group) ;

       insert into stg01.tb_msg(phone,msg,bsms,bkk,bcall)
       select v_mobile,
              substrb(regexp_replace(stg01.gettd()||':'||i_msg,';$','')||';'||chr(10)||to_char(sysdate,'YYYY-MM-DD HH24:MI:SS')||chr(10),1,450),
              --1���������������������������30���������,������������������
              case when (select count(1) from stg01.tb_msg where bsms<>'N' and dw_clt_date>=sysdate-1/24/60)>=30 then 'N' else decode(substr(i_sendtype,1,1),'1','Y','N') end,
