CREATE OR REPLACE procedure STG01.sp_imp_alone(i_kind in varchar2,i_sp_id in varchar2:='',i_value1 in varchar2:='')
as
...
begin
   ...
   if i_kind = 'plan_start' then
      --盘后采集
      for c1 in (--盘后定时采集
                 select sysid from stg01.vw_imp_system
                  where sys_kind='etl'
                    and bvalid=1
                    and stg01.fn_imp_pntype(db_start_type)=1
                    and to_char(v_curtime, 'HH24MI') not between '0001' and '0023'
                    and stg01.fn_imp_timechk(v_curtime,db_start) = 1
                    and i_sp_id is null
                  union all
                 --手工指定数据源采集
                 select sysid from stg01.vw_imp_system
                  ...
                  union all
                 --已经完成采集前初始化的判断标志数据源
                 select fid from stg01.tb_imp_flag
      ...
      loop
          ...
          stg01.sp_sms( '数据源[' || to_char(stg01.fn_imp_value('sysname',c1.sysid)) || ']正式开始采集!!' || v_remark , '1' , strtmp ) ;

          --数据源标志处理:只保留开始标志
          stg01.sp_imp_flag('del','ETL_START,ETL_END,TASK_GROUP',c1.sysid);
          stg01.sp_imp_flag('add','ETL_START',c1.sysid,3);
      ...
      loop
        stg01.sp_imp_flag('add','ETL_START',c1.sysid,0) ;
        stg01.sp_sms( '判断标志的数据源[' || c1.sysid||'_'||c1.sys_name || ']已经符合采集条件' ) ;
        v_sp_id := v_sp_id || c1.sysid || ',' ;
      end loop ;
      ...
      update stg01.tb_imp_etl
       ...
       where flag not in('R','X')
         and stg01.fn_imp_timechk(v_curtime, after_retry_fixed) = 1
         and stg01.fn_imp_pntype(after_retry_pntype) = 1 ;
      --计划调起数据服务
      update stg01.tb_imp_ds2
         set flag='N',retry_cnt=3
       where flag not in('R','X')
         and stg01.fn_imp_timechk(v_curtime, pn_fixed, pn_interval, pn_range) = 1
         and stg01.fn_imp_pntype(pn_type) = 1
         and (stg01.fn_imp_freqchk(run_freq) = 1 or run_freq = 'D') ;
      --计划调起SP计算
      ...
      if to_char(v_curtime, 'HH24MI') between '0800' and '2300' and to_char(v_curtime, 'YYYYMMDD') in (gettd(),getntd()) then
        update stg01.tb_imp_etl t
           set flag = 'N', retry_cnt = 3, etl_kind = 'R'
         where (
               stg01.fn_imp_timechk(v_curtime,realtime_fixed,realtime_interval,realtime_interval_range) = 1 or
               (realtime_taskgroup is not null and instr(','||realtime_taskgroup||',' , ','||nvl(i_sp_id,'0')||',')>0)
               )
           and flag in ('E', 'Y', 'N');

        --删除符合本次实时采集表的任务组完成标志
        delete stg01.tb_imp_flag
   ...
   --符合条件的任务开始执行
      ...
      --符合执行条件的任务组
       for c1 in(select a.task_group
                from (
                  select task_group from stg01.vw_imp_taskgroup_over  --常规任务组
                  union all
      ...
      loop
        --数据服务
        update stg01.tb_imp_ds2 set flag = 'N', retry_cnt = 3
         where task_group = c1.task_group and flag in('Y','E') and stg01.fn_imp_freqchk(run_freq)=1;
        --SP计算
        update stg01.tb_imp_sp set flag = 'N' , retry_cnt = 3 , runtime = runtime * power(2,3-retry_cnt)
         where realtime_taskgroup = c1.task_group and flag in('Y','E') and stg01.fn_imp_freqchk(run_freq)=1;
        --写任务组完成标志
        insert into stg01.tb_imp_flag(tradedate,kind,fid,fval) values ( v_tradedate , 'TASK_GROUP' , c1.task_group , 1 ) ;
      ...
      update stg01.tb_imp_sp set flag='Y'
      ...
      --重复执行的sp，删除任务组的完成标志
      delete from stg01.tb_imp_flag
       where tradedate = v_tradedate and kind = 'TASK_GROUP'
         and instr( (select ','||wm_concat(task_group)||',' from (select task_group from stg01.tb_imp_sp where flag='N' and task_group is not null group by task_group) ) , ','||fid||',' ) >0 ;
   ...
   --数据源采集结束
      ...
      stg01.sp_sms( '数据源[' || to_char(stg01.fn_imp_value('sysname',i_sp_id)) || ']采集结束'||chr(10)||v_remark ,'UF' , strtmp ) ;
      insert into stg01.tb_imp_flag(tradedate,kind,fid,fval) values ( v_tradedate , 'ETL_END' , i_sp_id , decode(v_err , 0 , 4 , 5 ) ) ;
   ...
   --实时与盘后的两个关键时间点任务处理
      ...
      update tb_imp_etl set bupdate='Y'
       where i_sp_id = 'R' and tid in(select tid from stg01.vw_imp_tbl_diff_hive union select tid from stg01.vw_imp_tbl_diff_mysql) ;
      ...
      for c1 in(select '采集模式置为' || i_sp_id || chr(10) || replace(wm_concat(sms), ',', chr(10)) sms
                  from (select sou_sysid || ':' || listagg(dest_tablename || chr(10)) within group(order by dest_tablename) sms
                         ...
                         group by sou_sysid)
                 union all
                select stg01.fn_imp_value('taskname',tid)||'的表有'||count(1)||'个字段类型变更!!!' from stg01.vw_imp_tbl_diff_hive
                 where substr(alter_sql,1,300) like '%` change `%' group by tid )
      loop
          stg01.sp_sms(substr(c1.sms,1,400),'1','110') ;
      end loop ;
   ...
   --更新与建表的过程处理
      ...
      update stg01.tb_imp_etl set bupdate='Y'
       where rowid in (select rid from stg01.vw_imp_etl where sou_db_conn = i_sp_id and bupdate <> 'Y' and i_value1 = 'Y') ;

      --获取完表结构:n
      update stg01.tb_imp_etl set bupdate='n'
       where rowid in (select rid from stg01.vw_imp_etl where sou_db_conn = i_sp_id and bupdate = 'Y' and i_value1 = 'n') ;
      ...
      if i_sp_id = 'etl_err' then
        ...
        loop
           ...
           stg01.sp_sms(to_char(stg01.fn_imp_value('sysname',c1.fid))||'采集失败,尝试自动修复','1','110');
        end loop ;
      ...
      --更新表结构:N updt_json
        ...
        update stg01.tb_imp_etl
           set bcreate = 'N'
         where rowid in (select rid from stg01.vw_imp_etl where bcreate = 'Y' and bupdate = 'n' and bvalid = 1 and tid in (select tid from tb_imp_tbl_hdp group by tid));

        --删除本次更新表涉及的命令列表(99,100为自动生成，其他的idx可以自定义)
        delete stg01.tb_imp_sp_com
         where com_idx in(99,100)
           and sp_id in(select tid from stg01.tb_imp_etl where bupdate = 'n' );
        --99:分区
        insert into stg01.tb_imp_sp_com(sp_id,com_idx,com_kind,com_text)
        select tid,99,'hive',
        ...
        --100:datax采集的json
        insert into stg01.tb_imp_sp_com(sp_id,com_idx,com_kind,com_text)
        select tid,100,'addax',stg01.fn_imp_value('jobfile',tid)
          from stg01.tb_imp_etl
         where bupdate = 'n' ;
      ...
      --数据服务更新涉及表
        ...
        v_remark := to_char(stg01.fn_imp_value('get_schema')) ;
        delete from stg01.tb_imp_sp_needtab where kind='DS' and i_value1 in(sp_id,'all') ;
        for ds in (select ds_id from stg01.vw_imp_ds2 where bvalid=1 and (ds_id = i_value1 or i_value1 = 'all'))
        loop
          for c1 in(select regexp_replace(upper(' '||sou_table),'[^A-Z0-9_]{1}(HBASE|CLICKHOUSE|MY_JSC|MY_ZG)\.',' XXX_') coms
                      from stg01.vw_imp_ds2_mid where ds_id = ds.ds_id and bvalid = 1 and sou_ishdp = 1 and sou_allsql=0)
          loop
            ctmp := c1.coms ;
            for c2 in 1..500
            loop
              ...
              if strtmp is not null then
                insert into stg01.tb_imp_sp_needtab(sp_id,table_name,kind)
   ...
   --系统检测:整合原有soutab_chk及其他
      ...
      --生成所有的检测结果
      delete from stg01.tb_imp_chk where chk_kind in(select chk_kind from stg01.tb_imp_chk_inf where engine='ini');
      for inf in (select t.*,rowid as rid from stg01.tb_imp_chk_inf t where bpntype=1 and engine='ini')
      loop
          update stg01.tb_imp_chk_inf set start_time = current_timestamp where rowid = inf.rid ;
          execute immediate 'insert into stg01.tb_imp_chk(chk_mobile,chk_sendtype,chk_kind,chk_name,chk_content)' || inf.chk_sql ;
          update stg01.tb_imp_chk_inf set end_time = current_timestamp where rowid = inf.rid ;
      end loop ;
      delete from stg01.tb_imp_chk where chk_kind not in(select chk_kind from stg01.tb_imp_chk_inf);
      ...
      loop
          stg01.sp_sms(c1.chk_content,c1.chk_mobile,c1.chk_sendtype);
      end loop ;
   ...
   --获取最新的hadoop表结构(不含视图)
     ...
     delete from stg01.tb_imp_etl_tbls where db_name||'.'||tbl_name in(select pkid from tmp_imp) ;
     insert into stg01.tb_imp_etl_tbls
     select * from stg01.tb_imp_etl_tbls_tmp where db_name||'.'||tbl_name in(select pkid from tmp_imp) ;

   --刷新ODS采集表的源和目标结构,用于字段对比
   elsif i_kind = 'colexch_updt' then
      --本次需要更新的表(如果没有指定更新，则是需要更新全部)
      insert into stg01.tmp_imp(pkid)
      select tid from stg01.vw_imp_etl where bupdate='n' ;
      select count(1) into v_err from stg01.tmp_imp ;

      --hdp
      delete from stg01.tb_imp_tbl_hdp where tid in(select pkid from stg01.tmp_imp) or v_err=0 or tid not in(select tid from stg01.tb_imp_etl);
      insert /*+append nologging*/ into stg01.tb_imp_tbl_hdp(tid,hive_owner,hive_tablename,col_name,col_type_full,col_type,col_precision,col_scale,col_idx,tbl_comment,col_comment,cd_id)
      ...
      --sou
      delete from stg01.tb_imp_tbl_sou where tid in(select pkid from stg01.tmp_imp) or v_err=0 or tid not in(select tid from stg01.tb_imp_etl);
      insert /*+append nologging*/ into stg01.tb_imp_tbl_sou(tid,sou_db_conn,sou_owner,sou_tablename,column_name_orig,column_name,column_id,data_type,data_length,data_precision,data_scale,tbl_comment,col_comment,dest_type,dest_type_full)
...
exception
   when others then
        stg01.sp_sms('sp_imp_alone执行报错,kind=['||i_kind||'],sp_id=['||i_sp_id||'],i_value1='||i_value1||'],错误说明=['||sqlerrm||']','18692206867','110') ;
end;