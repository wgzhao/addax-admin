CREATE OR REPLACE FUNCTION STG01.sp_imp_deal(i_kind varchar, i_key varchar DEFAULT '')
RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  FROM (
    ...
    FROM stg01.vw_imp_system_allsql
  ...
  IF i_kind = 'git_deploy' AND length(COALESCE(i_key, ' ')) = 32 THEN
    INSERT INTO stg01.tmp_ci_deploy_add
      ...
      SELECT COALESCE(a.sp_id, gen_random_uuid()::text) AS sp_id,
        ...
        FROM t_change x
        LEFT JOIN stg01.tb_ci_deploy t ON t.dep_id = i_key AND x.file_name = lower(t.file_name)
        LEFT JOIN stg01.tb_imp_sp a ON a.sp_owner || '.' || a.sp_name = regexp_replace(x.file_name, '\.sql$', '')
       WHERE x.add_kind < 9
    ...
    -- 其他复杂逻辑的简化处理
    PERFORM stg01.sp_sms('git发起代码提交!! 处理完成', '1', '010');
...
EXCEPTION
   WHEN OTHERS THEN
        PERFORM stg01.sp_sms('sp_imp_deal执行报错,kind=[' || i_kind || '],key=[' || i_key || '],错误说明=[' || substring(SQLERRM FROM 1 FOR 200) || ']', '18692206867', '110');
        RAISE;
END;
$$ LANGUAGE plpgsql;