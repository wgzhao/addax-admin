...
CREATE OR REPLACE FORCE VIEW "VW_CI_DEPLOY" ("DEP_ID", "PROJ_NAME", "FILE_PATH", "FILE_NAME", "FILE_CONTENT", "FILE_MD5", "UPDT_DATE", "SPNAME", "BVALID") AS
  select t.dep_id,
       t.proj_name,
       t.file_path,
       t.file_name,
...
CREATE OR REPLACE FORCE VIEW "VW_CI_DEPLOY_ID" ("DEP_ID", "DEP_DATE", "LAST_DEP_ID", "NEXT_DEP_ID") AS
  select dep_id,
       max_date dep_date,
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_CHECK_SOUTAB" ("SOU_DB_CONN", "OWNER", "TABLE_NAME", "COLUMN_NAME", "DATA_TYPE", "DATA_TYPE_LAST", "HIVE_TYPE", "HIVE_TYPE_LAST", "DATA_LENGTH", "DATA_LENGTH_LAST", "DATA_PRECISION", "DATA_PRECISION_LAST", "DATA_SCALE", "DATA_SCALE_LAST", "DW_CLT_DATE", "DW_CLT_DATE_LAST") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_CHK_INF" ("ENGINE", "CHK_IDX", "CHK_SENDTYPE", "CHK_MOBILE", "BPNTYPE", "CHK_KIND", "CHK_SQL") AS
  ...
  t_all as	--自定义配置项+配置主表内容变更
    ...
    select a.chk_idx,a.chk_sendtype,a.chk_mobile,'3',a.chk_kind,
      'select '''||table_name||''' chk_name,to_char(fn_imp_value(''taskname'',t.'||pk||')||''的'||
     ...
     from (
       select 'select '''||table_name||''' chk_name,fn_imp_value(''taskname'',nvl(t.'||pk||',a.'||pk||')) || case when t.'||pk||' is null then ''今日被删除---'' when a.'||pk||' is null then ''今日新增+++'' end chk_content '||
...
select engine,
       ...
       chk_sendtype,
       chk_mobile,
       fn_imp_pntype(chk_pntype) bpntype,
       chk_kind,
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_DATE" ("DT", "DT_FULL") AS
  select to_number(entry_value) as dt,entry_content as dt_full from tb_dictionary
where entry_code='1022'
;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_DB" ("DB_NAME", "DB_CONSTR", "DB_ID_ETL", "DB_USER_ETL", "DB_PASS_ETL", "DB_PARAL_ETL", "DB_ID_DS", "DB_USER_DS", "DB_PASS_DS", "DB_PARAL_DS", "DB_START", "DB_START_TYPE", "DB_JUDGE_SQL", "DB_JUDGE_PRE", "DB_REMARK", "DID", "BVALID", "SYS_NAME", "NETCHK", "DB_KIND", "DB_KIND_FULL", "CONF") AS
  select t.db_name,
       t.db_constr,
       t.db_id_etl,
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_DS2" ("DS_ID", "DS_NAME", "DEST_SYSID", "TASK_GROUP", "PARAM_SOU", "RETRY_CNT", "RUN_FREQ", "BVALID", "FLAG", "START_TIME", "END_TIME", "RUNTIME", "BRUN", "BDELAY", "BFREQ", "BPLAN", "PRE_SH", "POST_SH", "PRE_SQL", "POST_SQL", "INIT_RDS") AS
  select t.ds_id,a.ds_name,t.dest_sysid,t.task_group,t.param_sou,t.retry_cnt,t.run_freq,
       ...
       fn_imp_freqchk(run_freq) bfreq,
       case when pn_type is not null and (pn_fixed is not null or pn_interval is not null) then 1 else 0 end bplan,
       case when pre_sh is not null then fn_imp_param_replace(pre_sh,param_sou) end pre_sh,
       case when post_sh is not null then fn_imp_param_replace(post_sh,param_sou) end post_sh,
       case when nvl(pre_sql,'T') not in('T','D') or t.dest_sysid='TM' then a.d_conn_full || chr(10) || decode(dest_sysid,'TM',a.pre_bak,fn_imp_param_replace(pre_sql,param_sou)) end pre_sql,
       case when post_sql is not null then a.d_conn_full || chr(10) || replace(replace(fn_imp_param_replace(post_sql,param_sou),'${task_group}',task_group),'${start_time}',to_char(start_time,'YYYY-MM-DD HH24:MI:SS')) end post_sql,
       --redis
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_DS2_NEEDS" ("DS_ID", "TASK_GROUP", "FLAG", "START_TIME", "END_TIME", "BMULTI", "NEEDS", "BOVER") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_ETL" ("WKF", "SYSID", "SYS_NAME", "SYSNAME", "DB_START", "DB_START_DT", "SOU_DB_CONN", "SOU_DB_KIND", "SOU_DB_CONSTR", "SOU_DB_USER", "SOU_DB_PASS", "SOU_DB_CONF", "SOU_OWNER", "SOU_TABLENAME", "SOU_FILTER", "SOU_SPLIT", "DEST_OWNER", "DEST_TABLENAME", "DEST_PART_KIND", "FLAG", "PARAM_SOU", "BUPDATE", "BCREATE", "ETL_KIND", "RETRY_CNT", "START_TIME", "END_TIME", "RUNTIME", "RUNTIME_ADD", "TID", "RID", "SPNAME", "DEST", "BPREVIEW", "BTDH", "BREALTIME", "REALTIME_INTERVAL", "REALTIME_INTERVAL_RANGE", "REALTIME_TASKGROUP", "REALTIME_FIXED", "BAFTER_RETRY", "AFTER_RETRY_FIXED", "AFTER_RETRY_PNTYPE", "BRUN", "BVALID", "BCJ", "JOBKIND") AS
  select 'WF_ODS'||sou_sysid||'_ETL' wkf,
       ...
       case when t.sou_tablename like '%${%' then to_char(fn_imp_param_replace(t.sou_tablename,t.param_sou)) else t.sou_tablename end as sou_tablename,
  ...
  from tb_imp_etl t
 inner join vw_imp_tradetime x
    on 1 = 1
 inner join vw_imp_system a
    on a.sysid = t.sou_sysid
   and a.sys_kind = 'etl';
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_ETL_COLS" ("TID", "JOBKIND", "DATA_TYPE", "COLUMN_NAME", "BQUOTA", "COL_NAME", "COL_TYPE", "COL_IDX", "DBID") AS
  select t.tid,x.jobkind,a.data_type,a.column_name_orig as column_name, --必须用原始字段名
       case when c.entry_value is not null or upper(a.column_name_orig)<>a.column_name or regexp_like(a.column_name_orig,'^[0-9]+$') then 1 else 0 end bquota, --字段名修改了或者特殊字符，JSON中需要加引号       lower(t.col_name) col_name,lower(t.col_type) col_type,t.col_idx,x.sysid as dbid  from tb_imp_tbl_hdp t inner join vw_imp_etl x on x.tid=t.tid  left join tb_imp_tbl_sou a on a.tid=t.tid and a.column_name=t.col_name  left join tb_dictionary c on c.entry_code='2014' and c.entry_value=a.column_name --数据库关键字 where t.col_idx<1000;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_ETL_COLTYPE" ("COLTYPE", "HIVE_TYPE", "REMARK") AS
  select entry_value coltype,entry_content hive_type,remark from tb_dictionary t
where entry_code='2011'
;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_ETL_JUDGE" ("SYSID", "SYS_NAME", "DB_NAME", "JUDGE_SQL", "JUDGE_PRE", "BSTART", "FVAL", "JUDGE_TIME", "PX", "PARAM_VALUE", "DB_CONN") AS
  select t.sysid,
       t.sys_name,
       t.db_name,
       to_char(fn_imp_param_replace(t.db_judge_sql)) judge_sql,
       to_char(fn_imp_param_replace(t.db_judge_pre)) judge_pre,
       nvl(c.fval,-1) bstart,
       a.fval,
...
left join tb_imp_flag a on a.kind='ETL_JUDGE' and a.fid=t.sysid and a.tradedate=x.param_value
left join (select fid,fval,row_number() over(partition by fid order by dw_clt_date desc) px from vw_imp_flag where kind='ETL_START') c on c.fid=t.sysid and c.px=1
where t.sys_kind = 'etl' and t.db_judge_sql is not null
;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_ETL_OVERPREC" ("SYSNAME", "DB_START", "DB_START_DT", "TOTAL_CNT", "OVER_CNT", "OVER_PREC", "RUN_CNT", "ERR_CNT", "NO_CNT", "WAIT_CNT", "START_TIME_LTD", "END_TIME_LTD", "START_TIME_TD", "END_TIME_TD", "START_TIME_R", "END_TIME_R", "RUNTIME_LTD", "RUNTIME_TD", "RUNTIME_R") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_ETL_SOUTAB" ("KIND", "SOU_DB_CONN", "COL_JSON") AS
  with t_sql as  --获取（采集源库，数据服务目标库）的表结构信息
     (select kind,sou_db_conn,
      ...
      from (select 'etl' as kind,sou_db_conn,sou_db_kind,sou_owner,sou_tablename,
           ...
           union all
           select 'ds',sou_db_conn,db_kind_full,dest_owner,dest_tablename,null from vw_imp_ds2_mid
  ...
  (select 'ds' as kind,ds_id as sou_db_conn,'tb_imp_etl_tbls' dest_tbl,'jdbc:trino://infa01:18080/hive' sou_conn,'' sou_pass,
          'delete from tb_imp_etl_tbls where db_name=''ds'' and tbl_name in('||regexp_replace(listagg(''''||dsview||''',')within group(order by dsview),',$',')') pre_sql,
          '' post_sql,
   ...
   select 'etl','hadoop','tb_imp_etl_tbls_tmp','jdbc:mysql://nn01:3306/hive','ZEQEJGsNP7NT',
           'truncate table tb_imp_etl_tbls_tmp',
           'begin sp_imp_alone(''get_hdptbls'');end;',
...
select t.kind,t.sou_db_conn,
       replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(to_clob(c.entry_content),
           ...
           '${d_conn}','jdbc:oracle:thin:@inidb:1521/ini'),'${d_user}','stg01'),'${d_pass}','stg01pwd'),'${d_tblname}','tb_imp_etl_soutab'),
...
inner join tb_dictionary c on c.entry_code='5005' and c.entry_value='DS_JSON'
where b.bvalid=1	--数据源必须正常，否则获取表结构会失败
union all
--获取hadoop所有视图及表的字段信息
select kind,sou_db_conn,
       replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(
                               (select to_clob(entry_content) from tb_dictionary where entry_code='5005' and entry_value='DS_JSON'),
             '${s_conn}',sou_conn),'${s_user}','hive'),'${s_pass}',sou_pass),'${s_sql}',colsql),
             '${d_conn}','jdbc:oracle:thin:@inidb:1521/ini'),'${d_user}','stg01'),'${d_pass}','stg01pwd'),'${d_tblname}',''||dest_tbl),
             '${d_pre}',pre_sql),'${d_post}',post_sql)
from t_hdp;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_FLAG" ("TRADEDATE", "KIND", "FID", "FVAL", "DW_CLT_DATE") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_JOBFILE" ("JOBKIND", "JOBFILE") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_JY_NEEDBAK" ("TBL") AS
  select lower(dest_tablename) as tbl from tb_imp_etl t
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_PARAM" ("PARAM_SOU", "PARAM_KIND_0", "PARAM_KIND", "PARAM_NAME", "PARAM_REMARK", "PARAM_VALUE") AS
  select param_sou,
       param_kind_0,
       param_kind,
       param_name,
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_PARAM_FILE" ("PARAM_FILE", "PARAM_CONTENT") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_PLAN" ("PN_ID", "PN_TYPE", "PN_TYPE_NAME", "PN_FIXED", "DT_FULL", "PN_INTERVAL", "PN_RANGE", "SPNAME", "FLAG", "START_TIME", "END_TIME", "RUNTIME", "BRUN", "BPNTYPE") AS
  select pn_id,
       pn_type,
       fn_imp_pnname(pn_type) pn_type_name,
       pn_fixed,
       b.dt_full,
       pn_interval,
       pn_range,
       fn_imp_pnname(pn_type,pn_fixed,pn_interval,pn_range) spname,
       case
         when t.start_time between x.td and x.ntd then
          t.flag
         else 'N'
       ...
       case when flag<>'R' and pn_type<>'9'
             ...
             and fn_imp_timechk(sysdate, pn_fixed, pn_interval, pn_range,nvl(bexit,'Y'))=1
       then 1 else 0 end brun,
       fn_imp_pntype(pn_type) bpntype
  from tb_imp_plan t
 inner join vw_imp_tradetime x on 1=1
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_PLAN_ALL" ("PN_TYPE", "DT_FULL", "PN_TYPE_NAME", "SPNAME", "PN_FLAG", "C1", "C1F", "C2", "C2F", "C3", "C3F", "C4", "C4F", "C5", "C5F", "C6", "C6F") AS
  with t_pn as
...
select pn_type,(select dt_full from vw_imp_date where dt=regexp_substr(pn_fixed,'\d+')),
       fn_imp_pnname(pn_type),fn_imp_pnname(pn_type,pn_fixed,pn_interval,pn_range),flag,'数据服务:'||to_char(fn_imp_value('taskname',ds_id)),flag,
       null,null,null,null,null,null,null,null,null,null
from tb_imp_ds2
where pn_type is not null and flag<>'X'
union all
select pn_type,(select dt_full from vw_imp_date where dt=regexp_substr(pn_fixed,'\d+')),
       fn_imp_pnname(pn_type),fn_imp_pnname(pn_type,pn_fixed,pn_interval,pn_range),flag,'SP计算:'||to_char(fn_imp_value('taskname',sp_id)),flag,
       null,null,null,null,null,null,null,null,null,null
from tb_imp_sp
where pn_type is not null and flag<>'X'
order by 1,2,4
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_REALTIMES" ("TID", "TIMES") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_REALTIMES_ETL" ("LAST_TIMES", "NEXT_TIMES", "SPNAME", "START_TIME", "END_TIME", "FLAG") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_SP" ("SP_OWNER", "SP_NAME", "SP_ID", "RID", "FLAG", "RUN_FREQ", "START_TIME", "END_TIME", "RETRY_CNT", "RUNTIME", "REALTIME_TASKGROUP", "NEED_SOU", "NEED_SP", "SP_ALLTABS", "SP_DEST", "THROUGH_NEED_SOU", "THROUGH_NEED_SP", "TASK_GROUP", "PARAM_SOU", "REMARK", "SPNAME", "BVALID", "BRUN", "BFREQ", "BPLAN") AS
 ...
 inner join vw_imp_tradetime x
    on 1 = 1
  left join tb_imp_sp_needall a
    on a.sp_id = t.sp_id
;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_SP_DEST_EQUAL" ("SP_ID", "DEST") AS
  with t_dest as
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_SP_NEEDS" ("SP_ID", "FLAG", "START_TIME", "NEEDS", "NEEDS_FLAG", "NEEDS_END_TIME") AS
  with t_need_run as
 ( --SP执行情况
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_SP_SOURCE" ("PARENT_ID", "CATE_ID", "CATE_NAME", "CATE_VALUE") AS
  with t_sp as
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_SYSTEM" ("SYS_KIND", "SYSID", "SYS_NAME", "DB_NAME", "DB_CONSTR", "NETCHK", "BVALID", "DB_USER", "DB_PASS", "DB_PARAL", "DB_KIND", "DB_KIND_FULL", "CONF", "START_KIND", "DB_START_TYPE", "DB_START", "DB_START_DT", "CREATE_DB", "DB_JUDGE_SQL", "DB_JUDGE_PRE", "DB_CONN", "DB_CONN_CMD") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_SYSTEM_ALLSQL" ("DB_KIND_FULL", "DB_CONSTR", "DB_USER", "DB_PASS", "SYSID") AS
  select db_kind_full,
       db_constr,
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_TASKGROUP" ("TASK_GROUP") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_TASKGROUP_DETAIL" ("TASK_GROUP", "KIND", "ALLCNT", "YCNT", "START_TIME", "END_TIME", "RUNTIME", "RCNT", "START_TIME_R", "NCNT", "ECNT", "PREC", "KIND2", "TASK_GROUP2", "DS_NAME", "FLAG2", "START_TIME2", "END_TIME2", "BFLAG", "FLAG_TIME", "ERRMSG") AS
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_TASKGROUP_OVER" ("TASK_GROUP") AS
  select a.task_group
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_TBL_DIFF_HIVE" ("KIND", "TID", "ALTER_SQL") AS
  select '增加字段' as kind,tid,       lower('alter table `' || (select replace(dest,'.','`.`') from vw_imp_etl where tid=t.tid) ||'` add columns(' ||       wm_concat('`' || column_name || '` ' || dest_type_full || case when nvl(t.column_comment,' ')<>' ' then ' comment "'||t.column_comment||'"' end ) ||       ');') alter_sqlfrom vw_imp_tbl twhere col_name is nullgroup by tidunion allselect '字段类型修改' as kind,t.tid,      to_clob(lower('alter table '||a.hive_owner||'.`'||a.hive_tablename||'` change `'||a.col_name||'` `'||a.col_name||'` '||       t.dest_type||       case when t.dest_type='DECIMAL' then '('||            case when t.data_precision>nvl(a.col_precision,0) or t.data_scale>nvl(a.col_scale,0) then                 case when t.data_precision-a.col_precision<t.data_scale-a.col_scale then least(38,a.col_precision+t.data_scale-a.col_scale) else t.data_precision end            end||','|| case when t.data_scale>nvl(a.col_scale,0) then t.data_scale else nvl(a.col_scale,0) end||')'       end || case when nvl(t.col_comment,' ')<>' ' then ' comment "'||t.col_comment||'"' end || ';' )) alter_sqlfrom tb_imp_tbl_sou tinner join tb_imp_tbl_hdp a on a.tid=t.tid and t.column_name=a.col_namewhere t.dest_type_full<>a.col_type_full and not      (        (t.dest_type in('INT','BIGINT') and a.col_type in('INT','BIGINT'))        or        (t.dest_type='DECIMAL' and a.col_type='DECIMAL' and a.col_precision>=t.data_precision and a.col_scale>=t.data_scale)      )union allselect '修改表注释',t.tid,       'alter table `' ||(select to_clob(lower(replace(dest,'.','`.`'))) from vw_imp_etl where tid=t.tid) ||       '` set tblproperties("comment"="' || t.table_comment || '");'from (select tid,max(tbl_comment) as table_comment from tb_imp_tbl_sou t group by tid) tinner join (select tid,max(tbl_comment) as tbl_comment from tb_imp_tbl_hdp group by tid      ) a on a.tid=t.tidwhere nvl(a.tbl_comment,' ') <> t.table_comment;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_TBL_DIFF_MYSQL" ("KIND", "TID", "ALTER_SQL") AS
  select '修改字段注释' kind,tid,
       to_clob('/*'||(select lower(dest) from vw_imp_etl where tid=t.tid)||'*/ '||
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_TBL_HDP" ("TID", "BUPDATE", "HIVE_OWNER", "HIVE_TABLENAME", "COL_NAME", "COL_TYPE_FULL", "COL_TYPE", "COL_PRECISION", "COL_SCALE", "COL_IDX", "TBL_COMMENT", "COL_COMMENT", "CD_ID") AS
  select t.tid,
       t.bupdate,
       ...
       col_idx,
       to_char(fn_imp_comment_replace(tbl_comment)) tbl_comment,
       to_char(fn_imp_comment_replace(col_comment)) col_comment,
...
CREATE OR REPLACE FORCE VIEW "VW_IMP_TBL_SOU" ("TID", "BUPDATE", "SOU_DB_CONN", "SOU_OWNER", "SOU_TABLENAME", "COLUMN_NAME_ORIG", "COLUMN_NAME", "COLUMN_ID", "DATA_TYPE", "DATA_LENGTH", "DATA_PRECISION", "DATA_SCALE", "TBL_COMMENT", "COL_COMMENT", "DEST_TYPE", "DEST_TYPE_FULL") AS
  with t_sou as
 (select t.tid,
         t.bupdate,
         t.sou_db_conn,
         ...
         case when upper(a.column_name) in(select upper(entry_value) from tb_dictionary where entry_code='2015' union all select 'LOGDATE' from dual) then upper(a.column_name)||'_RENAME'              when regexp_like(upper(a.column_name), '[^A-Z0-9_]+') then 'COLUMN_' || column_id         else upper(column_name) end column_name,         a.column_id,         case when a.data_type='NUMBER' and a.data_precision is null and a.data_scale=0 then 'INT'              else regexp_substr(a.data_type, '[a-zA-Z0-9_]+') end data_type,         a.data_length,         case when nvl(a.data_precision, 38) > 38 then 38 else nvl(a.data_precision, 38) end data_precision,         nvl(a.data_scale, 10) data_scale,         to_char(fn_imp_comment_replace(tab_comment)) tbl_comment,         --源库字段名为中文的，字段名改了后，原有中文名放在注释中         case when regexp_like(upper(a.column_name), '[^A-Z0-9_]+') then '=>' || column_name || '<=' else '' end ||              to_char(fn_imp_comment_replace(col_comment)) col_comment    from vw_imp_etl t   inner join tb_imp_etl_soutab a      on a.sou_db_conn = t.sou_db_conn     and upper(a.owner) = upper(t.sou_owner)     and upper(a.table_name) = upper(t.sou_tablename)     )select x.TID,x.BUPDATE,x.SOU_DB_CONN,x.SOU_OWNER,x.SOU_TABLENAME,x.COLUMN_NAME_ORIG,x.COLUMN_NAME,x.COLUMN_ID,x.DATA_TYPE,x.DATA_LENGTH,x.DATA_PRECISION,x.DATA_SCALE,x.TBL_COMMENT,x.COL_COMMENT,x.DEST_TYPE,		case         when dest_type = 'DECIMAL' then          dest_type || '(' || data_precision || ',' || data_scale || ')'         else          dest_type       end dest_type_full  from (select t.tid,               t.bupdate,               t.sou_db_conn,               t.sou_owner,               t.sou_tablename,               t.column_name_orig,               t.column_name,               t.column_id,               t.data_type,               t.data_length,               t.data_precision,               t.data_scale,               t.tbl_comment,               t.col_comment,               upper(nvl(c.hive_type, 'string')) dest_type          from t_sou t          left join vw_imp_etl_coltype c            on upper(c.coltype) = t.data_type) x;
;

CREATE OR REPLACE FORCE VIEW "VW_IMP_TRADETIME" ("TD", "NTD") AS
  with t_param as
...
CREATE OR REPLACE FORCE VIEW "VW_MOBILE_GROUP" ("GROUPID", "MOBILE", "USERNAME") AS
  with t_gp as
...
CREATE OR REPLACE FORCE VIEW "VW_TMP" ("DB_NAME", "DB_CONSTR", "DB_ID_ETL", "DB_USER_ETL", "DB_PASS_ETL", "DB_PARAL_ETL", "DB_ID_DS", "DB_USER_DS", "DB_PASS_DS", "DB_PARAL_DS", "DB_START", "DB_START_TYPE", "DB_JUDGE_SQL", "DB_JUDGE_PRE", "DB_REMARK", "DID", "BVALID", "CONF") AS
  select t."DB_NAME",t."DB_CONSTR",t."DB_ID_ETL",t."DB_USER_ETL",t."DB_PASS_ETL",t."DB_PARAL_ETL",t."DB_ID_DS",t."DB_USER_DS",t."DB_PASS_DS",t."DB_PARAL_DS",t."DB_START",t."DB_START_TYPE",t."DB_JUDGE_SQL",t."DB_JUDGE_PRE",t."DB_REMARK",t."DID",t."BVALID",t."CONF" from tb_imp_db t
inner join (select max(db_id_etl) as maxid from tb_imp_db
	) a on a.maxid=t.db_id_etl;
;

CREATE OR REPLACE FORCE VIEW "VW_TRADE_DATE" ("INIT_DATE", "PX") AS
...
CREATE OR REPLACE FORCE VIEW "VW_UPDT_RDS" ("RDS") AS
  select 'set param.'||param_kind_0||' "'||param_value||'"' rds
from vw_imp_param
where param_sou='C' and param_kind_0 is not null
union all
select 'set '||decode(entry_code,'1061','shname','1062','path','1063','com')||'.'||entry_value||' "'||entry_content||'"' from tb_dictionary
