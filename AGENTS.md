...
当需要总结时，把本轮会话的内容做一个总结，写入到 memory 当中，具体要求为:

1. 在 `.memory` 目录下创建一个当天日期（格式为`YYYY-MM-DD.md`）的文件，然后把总结写入到该文件中
2. 如果文件已存在，则追加内容，不要覆盖原有内容
3. 然后在 `.memory/index.md` 中添加一个索引，格式为 `[总结标题](./YYYY-MM-DD.md)`
